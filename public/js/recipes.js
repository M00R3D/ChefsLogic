// Deletion is handled by the site-wide confirm-delete-modal
// If modal is not present, fall back to simple fetch behavior
if (!document.getElementById('confirm-delete-modal')) {
  async function deleteRecipeFallback(recipeId, triggerButton) {
    const shouldDelete = window.confirm("Esta accion eliminara la receta de forma permanente. Deseas continuar?");
    if (!shouldDelete) return;
    triggerButton.disabled = true;
    try {
      const response = await fetch(`/api/recipes/${recipeId}`, { method: 'DELETE', credentials: 'same-origin' });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.message || 'No fue posible eliminar la receta.');
      const row = triggerButton.closest('tr') || triggerButton.closest('.card'); if (row) row.remove();
      if (window.ChefUI) window.ChefUI.showToast('Receta eliminada correctamente.', 'success');
    } catch (e) { if (window.ChefUI) window.ChefUI.showToast(e.message || 'Error al eliminar receta', 'error'); }
    finally { triggerButton.disabled = false; }
  }

  document.querySelectorAll('[data-delete-recipe]').forEach((button) => {
    button.addEventListener('click', () => deleteRecipeFallback(button.dataset.deleteRecipe, button));
  });
}
